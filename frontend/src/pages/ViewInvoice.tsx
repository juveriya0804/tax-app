...
export default function ViewInvoice() {
  ...
  const loadInvoice = async () => {
    try {
      const res = await fetchApi(`/billing/invoices/${id}`);
      ...
      setInvoice({
        id,
        invoiceNumber: id === '1' ? 'INV-3381' : id === '2' ? 'INV-2309' : 'INV-1003',
        status: id === '3' ? 'PAID' : 'UNPAID',
        issueDate: '2026-06-01',
        dueDate: '2026-07-01',
        amount: 1500.00,
        vatAmount: 75.00,
        totalAmount: 1575.00,
        customer: { name: id === '1' ? 'Acme Corp' : id === '2' ? 'Globex Inc' : 'Initech', email: 'contact@example.com', phone: '+1 234 567 8900' },
        items: [
          { id: 'i1', description: 'Web Design Services', quantity: 1, unitPrice: 1000.00 },
          { id: 'i2', description: 'Hosting (1 Year)', quantity: 1, unitPrice: 500.00 }
        ]
      });
    }
  };
  ...
  if (!invoice) {
    return <div style={{ padding: '40px', textAlign: 'center' }}>Loading Invoice Details...</div>;
  }
  ...
  return (
    <div style={{ padding: '0 0 40px 0', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        >
          <ArrowLeft size={24} style={{ marginRight: '8px' }} /> Back to Invoices
        </button>
      ...
      <div className="glass-panel" style={{ padding: '48px', background: 'white', color: '#333' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #f3f4f6', paddingBottom: '32px', marginBottom: '32px' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', margin: '0 0 8px 0', color: '#111827' }}>INVOICE</h1>
            <div style={{ fontSize: '1.1rem', color: '#6b7280' }}>#{invoice.invoiceNumber}</div>
            <div style={{ marginTop: '16px' }}>
              <span style={{ 
                padding: '4px 12px', 
                borderRadius: '12px', 
                fontSize: '0.85rem', 
                background: invoice.status === 'DRAFT' ? '#f3f4f6' : '#ecfdf5',
                color: invoice.status === 'DRAFT' ? '#374151' : '#059669',
                fontWeight: 600
              }}>
                {invoice.status}
              </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
          <div>
            <h4 style={{ color: '#6b7280', margin: '0 0 8px 0', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Invoice To:</h4>
            <h3 style={{ margin: '0 0 4px 0', color: '#111827', fontSize: '1.2rem' }}>{invoice.customer?.name || 'Walk-in Customer'}</h3>
            {invoice.customer?.email && <p style={{ margin: '0', color: '#6b7280', fontSize: '0.9rem' }}>{invoice.customer.email}</p>}
            {invoice.customer?.phone && <p style={{ margin: '0', color: '#6b7280', fontSize: '0.9rem' }}>{invoice.customer.phone}</p>}
          </div>

          <div style={{ textAlign: 'right' }}>
            <div style={{ marginBottom: '12px' }}>
              <span style={{ color: '#6b7280', fontSize: '0.9rem', marginRight: '16px' }}>Issue Date:</span>
              <span style={{ fontWeight: 600, color: '#111827' }}>{new Date(invoice.issueDate).toLocaleDateString()}</span>
            </div>
            <div>
              <span style={{ color: '#6b7280', fontSize: '0.9rem', marginRight: '16px' }}>Valid Until:</span>
              <span style={{ fontWeight: 600, color: '#111827' }}>{invoice.validUntil ? new Date(invoice.validUntil).toLocaleDateString() : '-'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ width: '300px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 8px', borderBottom: '1px solid #f3f4f6', color: '#4b5563' }}>
              <span>Tax (VAT)</span>
              <span>₹{parseFloat(invoice.vatAmount).toFixed(2)}</span>
            </div>
        ...
        <div style={{ marginTop: '60px', paddingTop: '24px', borderTop: '1px solid #e5e7eb', color: '#6b7280', fontSize: '0.9rem' }}>
          ...
          <p style={{ margin: 0, lineHeight: '1.5' }}>
            1. Invoice is valid until the specified "Valid Until" date.<br/>
      ...
      <div style={{ marginTop: '32px', display: 'flex', flexDirection: 'column', gap: '1px', background: '#e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
        ...
        <div style={{ background: 'white' }}>
          ...
          {advancedSettingsOpen && (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#4b5563', fontSize: '0.95rem' }}>Show SKU in Invoice</span>
              <div style={{ width: '40px', height: '24px', background: '#e5e7eb', borderRadius: '12px', position: 'relative', cursor: 'pointer' }}>
                <div style={{ width: '20px', height: '20px', background: 'white', borderRadius: '50%', position: 'absolute', top: '2px', left: '2px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}></div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#4b5563', fontSize: '0.95rem' }}>Show Serial Numbers in Invoice</span>
              <div style={{ width: '40px', height: '24px', background: '#e5e7eb', borderRadius: '12px', position: 'relative', cursor: 'pointer' }}>
                <div style={{ width: '20px', height: '20px', background: 'white', borderRadius: '50%', position: 'absolute', top: '2px', left: '2px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}></div>
              </div>
            </div>
        ...
        <div style={{ background: 'white' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#374151', fontWeight: 500 }}>
              <Palette size={20} color="#6b7280" /> Customize Invoice Design
            </div>
          ...
          {approvalHistoryOpen && (
            <div style={{ padding: '40px 24px', position: 'relative', overflow: 'hidden', minHeight: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '500px', background: 'rgba(255,255,255,0.7)', padding: '24px', borderRadius: '12px' }}>
                ...
                <p style={{ color: '#4b5563', fontSize: '1rem', lineHeight: '1.5', margin: '0 0 24px 0' }}>
                  Add this Invoice to a Workflow to initiate Approvals
                </p>
