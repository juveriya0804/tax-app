...
export default function CreateDeliveryChallan() {
  ...
  const loadDependencies = async () => {
    try {
      // Assuming CRM leads/customers are fetched here. Let's just use /crm/leads for now or fake it if no endpoint exists
      const resC = await fetchApi('/crm/leads').catch(() => ({ data: [] }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchApi('/delivery-challans', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: '0 0 20px 0', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <h1 className="text-gradient" style={{ fontSize: '2rem', margin: 0 }}>Create Delivery Challan</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-panel" style={{ padding: '32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '24px', marginTop: '32px' }}>
          <div>
            {/* Left side space - can be used for notes/terms later */}
          </div>
          ...
          <div style={{ background: 'rgba(248, 249, 250, 0.5)', borderRadius: '12px', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', color: 'var(--text-secondary)' }}>
              <span>Amount</span>
              <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>₹{items.reduce((acc, item) => acc + (Number(item.quantity) * Number(item.unitPrice)), 0).toFixed(2)}</span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-secondary)', marginBottom: '24px' }}>
              <input type="checkbox" style={{ width: '16px', height: '16px', accentColor: '#8b5cf6', cursor: 'pointer' }} />
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <span style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--text-primary)' }}>₹{items.reduce((acc, item) => acc + (Number(item.quantity) * Number(item.unitPrice)), 0).toFixed(2)}</span>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#8b5cf6', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '24px', marginTop: '16px' }}>
          <div></div>
          <div style={{ background: 'rgba(248, 249, 250, 0.5)', borderRadius: '12px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', fontWeight: 600, color: '#334155' }}>
            ...
            <input 
              ...
              style={{ width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid rgba(0,0,0,0.1)', borderRadius: 0, padding: '8px 0', fontSize: '1rem' }}
            />
          </div>
        </div>
