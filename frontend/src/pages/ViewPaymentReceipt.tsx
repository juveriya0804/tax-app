...
export default function ViewPaymentReceipt() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [receipt, setPaymentReceipt] = useState<any>(null);
    ...
    try {
      ...
      if (found) {
        setPaymentReceipt({
          ...
          status: 'PAID',
          issueDate: found.paymentDate,
          amount: parseFloat(found.amount),
          vatAmount: 0,
          totalAmount: parseFloat(found.amount),
          customer: { name: found.customerName, email: '', phone: '' },
          items: [{ 
            id: 'i1', 
            description: found.notes || 'Payment Received', 
            quantity: 1, 
            unitPrice: parseFloat(found.amount) 
          }]
        });
      } else {
        // Fallback for UI testing if hit directly with fake ID
        setPaymentReceipt({
          id,
          receiptNumber: 'PR-XXXX',
          status: 'PAID',
          issueDate: new Date().toISOString(),
          amount: 0,
          vatAmount: 0,
          totalAmount: 0,
          customer: { name: 'Unknown', email: '', phone: '' },
          items: []
        });
      }
    } catch (e) {
  ...
  if (!receipt) {
    return <div style={{ padding: '40px', textAlign: 'center' }}>Loading Payment Receipt Details...</div>;
  }
  ...
  return (
    <div style={{ padding: '0 0 40px 0', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        >
          <ArrowLeft size={24} style={{ marginRight: '8px' }} /> Back to PaymentReceipts
        </button>
      ...
      <div className="glass-panel" style={{ padding: '48px', background: 'white', color: '#333' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #f3f4f6', paddingBottom: '32px', marginBottom: '32px' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', margin: '0 0 8px 0', color: '#111827' }}>PAYMENT RECEIPT</h1>
            <div style={{ fontSize: '1.1rem', color: '#6b7280' }}>#{receipt.receiptNumber}</div>
            <div style={{ marginTop: '16px' }}>
              <span style={{ 
                padding: '4px 12px', 
                borderRadius: '12px', 
                fontSize: '0.85rem', 
                background: receipt.status === 'DRAFT' ? '#f3f4f6' : '#ecfdf5',
                color: receipt.status === 'DRAFT' ? '#374151' : '#059669',
                fontWeight: 600
              }}>
                {receipt.status}
              </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
          <div>
            <h4 style={{ color: '#6b7280', margin: '0 0 8px 0', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>PaymentReceipt To:</h4>
            <h3 style={{ margin: '0 0 4px 0', color: '#111827', fontSize: '1.2rem' }}>{receipt.customer?.name || 'Walk-in Customer'}</h3>
            {receipt.customer?.email && <p style={{ margin: '0', color: '#6b7280', fontSize: '0.9rem' }}>{receipt.customer.email}</p>}
            {receipt.customer?.phone && <p style={{ margin: '0', color: '#6b7280', fontSize: '0.9rem' }}>{receipt.customer.phone}</p>}
          </div>

          <div style={{ textAlign: 'right' }}>
            <div style={{ marginBottom: '12px' }}>
              <span style={{ color: '#6b7280', fontSize: '0.9rem', marginRight: '16px' }}>Issue Date:</span>
              <span style={{ fontWeight: 600, color: '#111827' }}>{new Date(receipt.issueDate).toLocaleDateString()}</span>
            </div>
            <div>
              <span style={{ color: '#6b7280', fontSize: '0.9rem', marginRight: '16px' }}>Valid Until:</span>
              <span style={{ fontWeight: 600, color: '#111827' }}>{receipt.validUntil ? new Date(receipt.validUntil).toLocaleDateString() : '-'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ width: '300px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 8px', borderBottom: '1px solid #f3f4f6', color: '#4b5563' }}>
              <span>Tax (VAT)</span>
              <span>₹{parseFloat(receipt.vatAmount).toFixed(2)}</span>
            </div>
        ...
        <div style={{ marginTop: '60px', paddingTop: '24px', borderTop: '1px solid #e5e7eb', color: '#6b7280', fontSize: '0.9rem' }}>
          ...
          <p style={{ margin: 0, lineHeight: '1.5' }}>
            1. PaymentReceipt is valid until the specified "Valid Until" date.<br/>
      ...
      <div style={{ marginTop: '32px', display: 'flex', flexDirection: 'column', gap: '1px', background: '#e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
        ...
        <div style={{ background: 'white' }}>
          ...
          {advancedSettingsOpen && (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#4b5563', fontSize: '0.95rem' }}>Show SKU in PaymentReceipt</span>
              <div style={{ width: '40px', height: '24px', background: '#e5e7eb', borderRadius: '12px', position: 'relative', cursor: 'pointer' }}>
                <div style={{ width: '20px', height: '20px', background: 'white', borderRadius: '50%', position: 'absolute', top: '2px', left: '2px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}></div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#4b5563', fontSize: '0.95rem' }}>Show Serial Numbers in PaymentReceipt</span>
              <div style={{ width: '40px', height: '24px', background: '#e5e7eb', borderRadius: '12px', position: 'relative', cursor: 'pointer' }}>
                <div style={{ width: '20px', height: '20px', background: 'white', borderRadius: '50%', position: 'absolute', top: '2px', left: '2px', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}></div>
              </div>
            </div>
        ...
        <div style={{ background: 'white' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#374151', fontWeight: 500 }}>
              <Palette size={20} color="#6b7280" /> Customize PaymentReceipt Design
            </div>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '32px', background: '#fafafa' }}>
              ...
              <div style={{ background: 'white', padding: '24px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                ...
                <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '12px', fontSize: '0.9rem', color: '#4b5563' }}>PaymentReceipt Color</label>
          ...
          {approvalHistoryOpen && (
            <div style={{ padding: '40px 24px', position: 'relative', overflow: 'hidden', minHeight: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
              ...
              <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '500px', background: 'rgba(255,255,255,0.7)', padding: '24px', borderRadius: '12px' }}>
                ...
                <p style={{ color: '#4b5563', fontSize: '1rem', lineHeight: '1.5', margin: '0 0 24px 0' }}>
                  Add this PaymentReceipt to a Workflow to initiate Approvals
                </p>
